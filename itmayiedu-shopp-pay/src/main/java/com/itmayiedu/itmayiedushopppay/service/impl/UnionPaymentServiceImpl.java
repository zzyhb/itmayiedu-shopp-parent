package com.itmayiedu.itmayiedushopppay.service.impl;

import com.itmayiedu.common.util.CommonUtil;
import com.itmayiedu.itmayiedushopppay.sdk.union.*;
import com.itmayiedu.itmayiedushopppay.service.UnionPaymentService;
...
public class UnionPaymentServiceImpl implements UnionPaymentService {
    ...
    //先取商户号
    private String merId = "777290058165456";

    private String redictUrl = "http://47.99.84.178:8081";
    ...
    public void pay(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        amount = amount.multiply(new BigDecimal(1)).setScale(0, BigDecimal.ROUND_HALF_UP);
        txnAmt = amount.toString();
        //订单发送时间用于查询用
        Date sendPaymentDate = new Date();
        txnTime = formatTime(sendPaymentDate);
        System.out.println("交易订单时间:"+txnTime);
        ...
        requestData.put("orderId", CommonUtil.getUUID());    //商户订单号，8-40位数字字母，不能含“-”或“_”，可以自行定制规则
    ...
    public void backRcvResponse(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (!AcpService.validate(valideData, encoding)) {
            ...
            if("00".equals(respCode)){  // 00 交易成功
                System.out.println("银联支付测试成功!");
                //queryIsSuccess(request,response,"123456789","20181211113053");
                //todo 若交易成功
    ...
    public void query(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        //String orderId = String.valueOf(System.currentTimeMillis()); //实际上是orderSn
        String orderId = String.valueOf(System.currentTimeMillis()); //订单号
        String txnTime = null; //订单发送时间
    ...
    public void queryIsSuccess(HttpServletRequest request, HttpServletResponse response,String orderId,String txnTime) throws IOException {
        ...
        if(!rspData.isEmpty()){
            if(AcpService.validate(rspData, DemoBase.encoding)){
                ...
                if("00".equals(rspData.get("respCode"))){//如果查询交易成功
                    ...
                    if("00".equals(origRespCode)){
                        //交易成功，更新商户订单状态
                        System.out.println("交易成功，可以更新商户订单状态");
                        //TODO
                    }else if("03".equals(origRespCode) ||
                            "04".equals(origRespCode) ||
                            "05".equals(origRespCode)){
